...
void WG_Player_Page::onInit(SDL_Renderer* renderer, Audio* sounds,int w, int h, int* ascreen)
{
    ...
    title0.Init( renderer, "new player", path + "future.ttf", 48, titleColor);
    ...
    quitButton.addText("quit", path + "future.ttf", white, bf);
    quitButton.addBackground(path + "red_button12.png", 190, 45);
};
...
void WG_Player_Page::onEvent(SDL_Event event)
{
    if (event.type == SDL_MOUSEBUTTONDOWN)
    {
        if(event.button.button = SDL_BUTTON_LEFT)
        {
            ...
            if(startGameButton.onClick(x, y) == 0) changeScreen(8);
            if(backButton.onClick(x, y) == 0) changeScreen(2);
            if(quitButton.onClick(x, y) == 0) changeScreen(1);
        }
    }
};
